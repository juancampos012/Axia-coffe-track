...
import { ArrowLeft, Truck, Building2, ShoppingBag } from "lucide-react";
...
export default function PurchaseDetailClient({ invoice }: PurchaseDetailClientProps) {
    ...
    const infoCards = [
        ...
        { label: "Total", value: formatCurrency(invoice.totalPrice || 0) },
    ];
            ...
            >
                <div className="flex items-center gap-2 mb-4">
                    <div className="p-1.5 rounded-lg" style={{ background: "rgba(74,127,255,0.1)" }}>
                        <Truck size={13} style={{ color: "#4a7fff" }} />
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.35)" }}>{t("supplierSection")}</span>
                </div>
                <div className="space-y-1.5 text-sm">
                    <p style={{ color: "rgba(255,255,255,0.7)" }}><span style={{ color: "rgba(255,255,255,0.35)" }}>{t("supplier.name")}: </span>{invoice.supplier.name}</p>
                    <p style={{ color: "rgba(255,255,255,0.7)" }}><span style={{ color: "rgba(255,255,255,0.35)" }}>{t("supplier.nit")}: </span>{invoice.supplier.nit}</p>
                    <p style={{ color: "rgba(255,255,255,0.7)" }}><span style={{ color: "rgba(255,255,255,0.35)" }}>{t("supplier.phone")}: </span>{invoice.supplier.phone}</p>
                    <p style={{ color: "rgba(255,255,255,0.7)" }}><span style={{ color: "rgba(255,255,255,0.35)" }}>{t("supplier.address")}: </span>{invoice.supplier.address}</p>
                </div>
            </div>
